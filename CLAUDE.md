...
**Current state**: Parts 1-9 of `docs/PLAN.md` are done — `backend/` is a real FastAPI app (auth, SQLite-backed board persistence, OpenRouter connectivity via `app/ai/openrouter.py`, and a `POST /api/chat` endpoint that turns chat messages into validated board operations via structured outputs), `frontend/` is wired to the auth/board APIs (login, logout, and the Kanban board all round-trip through the backend, no more in-memory-only state; the chat endpoint has no UI yet), and `scripts/` has working Docker start/stop scripts. A real `OPENROUTER_API_KEY` is in the root `.env` (gitignored, not committed). The AI chat sidebar UI (Part 10) is not started. Check `docs/PLAN.md` for exact per-part status.
